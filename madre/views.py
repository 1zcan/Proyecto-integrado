...
class MadreListView(ListView):
    ...
    def get_queryset(self):
        ...
        if comuna: qs = qs.filter(comuna_id=comuna)
        if cesfam: qs = qs.filter(cesfam_id=cesfam)
        return qs
...
class MadreDeleteView(LoginRequiredMixin, View):
    ...
    def get(self, request, pk):
        ...
        context = { "madre": madre, "hijos": hijos, "total_hijos": hijos.count(), "form": form }
    ...
    def post(self, request, pk):
        ...
        detalle = f"Eliminación de Madre ID={madre.id}, RUT={madre.rut}. Motivo: {razon}. Total RN eliminados: {hijos.count()}."
        
        LogAccion.objects.create(
            usuario=request.user, accion=LogAccion.ACCION_DELETE, modelo="Madre",
            objeto_id=str(madre.pk), detalle=detalle, ip_address=get_client_ip()
        )
